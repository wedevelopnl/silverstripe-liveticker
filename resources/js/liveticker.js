...
(function () {
  ...
  var FRAGMENT_SEPERATOR = "_"; // The autodeclare keyword.

  GraphQLClient.AUTODECLARE_PATTERN = /\(@autodeclare\)|\(@autotype\)/;
  GraphQLClient.FRAGMENT_PATTERN = /\.\.\.\s*([A-Za-z0-9\.\_]+)/g; // Flattens nested object
  ...
  GraphQLClient.prototype.flatten = function (object) {
    ...
    for (name in object) {
      if (object.hasOwnProperty(name)) {
        _typeof(object[name]) == "object" ? this.flatten(object[name], prefix + name + FRAGMENT_SEPERATOR, out) : out[prefix + name] = object[name];
      }
    }
  ...
  GraphQLClient.prototype.collectFragments = function (query, fragments) {
    ...
    (query.match(fragmentRegexp) || []).forEach(function (fragment) {
      ...
      if (fragment) {
        ...
        collectedFragments.push(fragment); // Collect sub fragments
  ...
  GraphQLClient.prototype.processQuery = function (query, fragments) {
    if (_typeof(query) == 'object' && query.hasOwnProperty('kind') && query.hasOwnProperty('definitions')) {
  ...
  GraphQLClient.prototype.autoDeclare = function (query, variables) {
    ...
    return query.replace(GraphQLClient.AUTODECLARE_PATTERN, function () {
      ...
      for (var key in variables) {
        ...
        var mapping = typeMap[_typeof(value)];
  ...
  GraphQLClient.prototype.createSenderFunction = function () {
    ...
    return function (query) {
      ...
      var caller = function caller(variables, requestOptions) {
